...
exports.Listener = (function() {
  ...
  QueryListener.prototype.start = function() {
    return hubCache.start().bind(this).then(startServer);
  };
  ...
  var startServer = function() {
    ...
    var timeSeriesHandler = function(req, res, next) {
      ...
      datasetAPI.getDataset(hubId, feedId, from, to, sortBy, sortDir, limit, skip, function(err, docs) {
        next.ifError(err);
        res.send(docs);
        next();
      });
    ...
    server.get("/timeseries/:hubId/:feedId", timeSeriesHandler);
    server.get("/timeseries/:hubId/:feedId/:from", timeSeriesHandler);
    server.get("/timeseries/:hubId/:feedId/:from/:limit", timeSeriesHandler);
    server.get("/timeseries/:hubId/:feedId/:from/:limit/:skip", timeSeriesHandler);
    server.get("/timeseries/:hubId/:feedId/:from/:to/:sortBy", timeSeriesHandler);
    server.get("/timeseries/:hubId/:feedId/:from/:to/:sortBy/:sortDir", timeSeriesHandler);
    server.get("/timeseries/:hubId/:feedId/:from/:to/:sortBy/:sortDir/:limit", timeSeriesHandler);
    server.get("/timeseries/:hubId/:feedId/:from/:to/:sortBy/:sortDir/:limit/:skip", timeSeriesHandler);
