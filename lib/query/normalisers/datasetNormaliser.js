...
exports.Normaliser = (function() {
  ...
  var loadHubFeed = function(hub, f) {
    ...
    return this._feedRepo.get(f.id).then(function(feed) {
      if (!feed.deleted) {
  ...
  var addHubFeeds = function(hub) {
    ...
    return feeds.find({ event: "created", "params.hubId": hub.id }).toArrayAsync().then(function (feeds) {
      if (feeds.length > 0) {
  ...
  DatasetNormaliser.prototype.get = function(hubId) {
