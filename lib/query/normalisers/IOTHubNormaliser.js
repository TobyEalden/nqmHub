...
exports.Normaliser = (function() {
  ...
  var feedCollection = "read.IOTFeed";
  ...
  IOTHubNormaliser.prototype.start = function() {
    ...
    self._hubs = db.collection(hubCollection);
    if (self._hubs) {
      self._hubs.drop();
    }

    self._feeds = db.collection(feedCollection);

    return self._hubs.createIndexAsync({ id: 1 }); //.bind(this).then(primeCache);
  };
  ...
  var hubHandler = function(eventName, evt) {
    var self = this;
    log("received hub event %s",eventName);
    ...
    switch (eventName) {
      ...
      case "Hub.deleted":
        ...
        break;
      case "Hub.created":
        getOrAdd.call(self, evt.id).bind(this).then(saveCache, errorCheck);
        break;
      case "Hub.renamed":
        getOrAdd.call(self, evt.id).bind(this).then(function(hub) {
          hub.replay([evt]);
          return hub;
        }).then(saveCache, errorCheck);
        break;
      default:
  ...
  var feedHandler = function(eventName, evt) {
    ...
    if (eventName === "Feed.snapshot") {
      log("received Feed snapshot event for %j", evt.key);
      delete evt.snapshot._id;
      self._feeds.updateOneAsync(evt.key, evt.snapshot, { upsert: true });
    } else {
      // Find the hub that owns this feed.
      getOrAdd.call(self, evt.hubId).then(function(hub) {
        if (!hub) {
          ...
          switch (eventName) {
            case "Feed.created":
              ...
              break;
            case "Feed.renamed":
              update = _.find(hub.feeds, function(f) { return f.id == evt.id; });
              if (update) {
  ...
  var saveCache = function(hub) {
    ...
    return this._hubs.updateOneAsync({ id: hub.id }, cleanHub, { upsert: true });
  };
  ...
  var getOrAdd = function(id) {
    ...
    if (!this._hubCache.hasOwnProperty(id)) {
      ...
      return Promise.resolve(this._hubCache[id]);
    }
  };

  var loadHub = function(h) {
    return getOrAdd.call(this, h.id).bind(this).then(saveCache, errorCheck);
  };
  ...
  var deleteFromCache = function(id) {
    ...
    return self._hubs.deleteOneAsync({id: id }).then(function() {
  ...
  IOTHubNormaliser.prototype.get = function(hubId) {
    return getOrAdd.call(this, hubId);
  };
