...
exports.Normaliser = (function() {
  ...
  FeedNormaliser.prototype.start = function() {
    ...
    eventBus.on("*.*.feedData",function(evt) {
      ...
      if (lookup.length > 1) {
        self._feedData.insertOne({id: lookup[1], hubId: lookup[0], datum: evt.params }, errorCheck);
      } else {
    ...
    self._feedData = mongo.queryDb.db().collection("read.IOTData");

    return self._feedData.createIndexAsync({ id: 1, hubId: 1 });
  };
