...
module.exports = (function() {
  ...
  var createDataset = function(params) {
    ...
    return this._datasetRepo.get(params.id).then(function(dataset) {
      ...
      if (dataset) {
        throw new Error("dataset already exists with key %j" + params.id);
      }
  ...
  var getDataRepository = function(dataset) {
    ...
    return {
      indices: indices,
      repo: new Repository(DatasetRow, { snapshotFrequency: 1, singleSnapshot: true, indices: indices, collectionBaseName: dataset.store })
    };
  };
  ...
  var dataCreate = function(params) {
    return this._datasetRepo.get(params.datasetId).then(function(dataset) {
      if (!dataset) {
        throw new Error("dataset not found with id %s",params.datasetId);
      }
      ...
      return storeRepo.repo.get(key).then(function(row) {
        ...
        var newRow = storeRepo.repo.factory();
        newRow.create(params);
        return storeRepo.repo.commit(newRow);
      });
    });
  };
  ...
  var dataMethod = function(method, params) {
    return this._datasetRepo.get(params.datasetId).then(function(dataset) {
      if (!dataset) {
        throw new Error(util.format("dataset not found with id %d", params.datasetId));
      }
      var storeRepo = getDataRepository(dataset);
      var key = [];
      _.forEach(storeRepo.indices, function(k) {
