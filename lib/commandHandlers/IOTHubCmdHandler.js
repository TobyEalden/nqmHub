...
module.exports = (function() {
  ...
  var nextDatum = function(i, data, cb) {
    var self = this;
    i++;
    if (i < data.length) {
      process.nextTick(function() { processFeedData.call(self, i, data, cb); });
    } else {
  ...
  var startProcessFeedData = function(params, cb) {
    ...
    this._hubRepo.get(params.hubId, function(err, hub) {
      if (err) {
        ...
        var sorted = _.sortBy(params.payload, "id");
        self._cachedFeedData = self._feedDataRepo = null;
        processFeedData.call(self, 0, sorted,  cb);
      }
    });
  };

  var processFeedData = function(i, data, cb) {
    var self = this;
    var datum = data[i];

    var processDatum = function() {
      self._cachedFeedData.create(datum);
      nextDatum.call(self, i, data, cb);
    };
